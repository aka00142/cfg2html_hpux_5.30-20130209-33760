#!/usr/bin/ksh
VERSION="cfg2html/HP-UX version "$(echo "@(#)$Revision: 5.30 $" | cut -f2 -d" ")
###########################################################################
# @(#) $Id: cfg2html_hpux.sh,v 5.30 2013-02-09 10:24:34 ralph Exp $
# set -vx
###########################################################################
...
CFG_BCSCONFIG="no";
CFG_DATE="";
CFG_DIAG="yes";         # fixed, 311003, sr by pk
...
CFG_KERNEL="yes"
CFG_LVM="yes"
CFG_MCSG="yes"  	#  13.03.2007, 12:08 modified by Ralph Roth
CFG_NETWORK="yes"
CFG_SAP="no"
CFG_SECURITY="no";
CFG_SOFTWARE="yes"
...
do
    case $Option in
        ...
        v     ) echo $VERSION"//"$(uname -mrs); exit 13;;   ## add uname output, see YG MSG 790 ##
...
line ( ) {
    ...
    echo "--=[ http://www.cfg2html.com ]=---------------------------------------------"
    # echo "--=[ "$(tput smso)"http://www.cfg2html.com"$(tput sgr0)" ]=-----------------------------------------------"
}
echo "\n"

#####################################################################
# @(#) Cfg2Html/HP-UX (c) by ROSE SWE, Dipl.-Ing. Ralph Roth
# http://rose.rult.at + http://www.cfg2html.com 
#####################################################################
# This is the "swiss army knife" for the ASE, TC, CE, sysadmin etc.
# I wrote it to get the necessary information to plan an update,
...
#####################################################################
# To Do/SR/Bugs, see doc/ToDo.txt
#####################################################################
# History (full history is in doc/history*.txt)
#####################################################################
# $Log: cfg2html_hpux.sh,v $
# Revision 5.30  2013-02-09 10:24:34  ralph
# replaced defect come.to redirector with rose.rult.at
#
# Revision 5.29  2013-02-08 21:48:34  ralph
# fixes for defect come.to redirector
#
# Revision 5.28  2012-12-28 11:00:04  ralph
# (c) y2k13 by Ralph Roth
#
# Revision 5.27  2012-07-09 18:59:15  ralph
# backport_by_BillHassell
#
# Revision 5.24  2011-12-28 09:41:36  ralproth
# cfg5.23-32068: Consolidated the (C)opyright messages to one common format, y2k12 header
#
# Revision 5.23  2011-12-28 09:33:48  ralproth
# cfg5.22-32061: y2k11 - changed the copyright
#
# Revision 5.22  2011-11-22 16:38:57  ralproth
# cfg5.21-32059: + Audit System (CoreOS), first entires
#
# Revision 5.21  2011-11-02 15:53:41  ralproth
# cfg5.20-32058: cmmakepkg-l
#
# Revision 5.20  2011-11-01 07:26:31  ralproth
# cfg5.19-31551: Many enhancements and SuperDome2 stuff submitted by Kathy Leslie
#
# Revision 5.19  2011-10-18 16:47:43  ralproth
# cfg5.18-31545: small documentation changes/enhancements
#
# Revision 5.18  2011-09-01 12:54:13  ralproth
# cfg5.17-31555: Fixes for quicklan.pl
#
# Revision 5.17  2011-08-31 13:32:18  ralproth
# cfg5.16-31551: qlan/perl/CIM/RSP fixes
#
# Revision 5.16  2011-08-31 11:45:11  ralproth
# cfg5.15-31551: First fix for pvgfilter
#
# Revision 5.15  2011-08-24 12:10:27  ralproth
# cfg5.14-31549: + rpcinfo enhancements
#
# Revision 5.14  2011-07-26 15:19:22  ralproth
# cfg5.13-31446: changed web url
#
# Revision 5.13  2011-06-06 19:15:45  ralproth
# cfg5.12-33155: OpenWRT: Fixes, removed some old remarks, added IP stuff/WLAN stuff
#
# Revision 5.12  2011-05-20 12:43:26  ralproth
# cfg4.97-25655: cfg2html 4.97 upstream (merged/updated)
#
# Revision 4.97  2011-05-18 14:03:44  ralproth
# cfg4.96-25654: fixes in the help screen
#
# ---------------------------------------------------------------------------
# Revision 4.10.1.1  2008/10/28 08:42:40  ralproth
# ---------------------------------------------------------------------------
# Revision 3.73  2008/10/28 08:42:40  ralproth
# Revision 3.10.1.1  2005/08/16 09:41:34  ralproth
# ---------------------------------------------------------------------------
...
<hr><small>Created "$DATEFULL" with " $VERSION "<br>
    " >> $HTML_OUTFILE
...
paragraph() {
    if [ "$HEADL" -le 1 ] ; then
        echo "\n<HR>\n" >> $HTML_OUTFILE_TEMP
        HEADL=1    #  15.4.2009, 13:07  Ralph Roth
    fi
    echo "<A NAME=\"$1\"></A>" >> $HTML_OUTFILE_TEMP
    echo "<H${HEADL}><A HREF=\"#Inhalt-$1\"> $1 </A></H${HEADL}><P>" >> $HTML_OUTFILE_TEMP
    
    # commented to eliminate the need of the gif, FMA
    #echo "<IMG SRC="profbull.gif" ALT="bull" WIDTH=14 HEIGHT=14>" >> $HTML_OUTFILE
...
######################################################################
#  Documents the single commands and their generated output
#  $1  = unix command,  $2 = text for the heading
######################################################################
...
close_html() {
    ...
    echo "\n<hr>\n" >> $HTML_OUTFILE
    echo "</P><P>\n<hr><FONT COLOR=blue><small>Created "$DATEFULL" with " $VERSION " (c) 1998- 2013 by "  >> $HTML_OUTFILE_TEMP
    ...
    echo "\n\nCreated "$DATEFULL" with " $VERSION " (c) 1998- 2013 by ROSE SWE\n" >> $TEXT_OUTFILE
}
...
get_LIF()
{
    ...
    for i in $boot
    do
        #boot2=`echo $i|sed "s/s[0-9].*/s6/p"|sed "s/dsk/rdsk/p"|grep rdsk`	## needs rework for HP-UX 11.31 MSS
        #boot2=`echo $i|sed "s/s[0-9].*/s6/"|sed "s/dsk/rdsk/"|grep rdsk`
    ...
    grep "Boot device" /var/adm/syslog/syslog.log | sort -u             # ER by TB, #  18.03.2011, 22:14 modified by Ralph Roth #* rar *#
...
LANG_C()        # reset local language to "C" -> English, default catalog
{
    export LANG="C"
    export LANG_ALL="C"
    export LC_MESSAGE="C"
...
line
echo "Starting          "$VERSION" on a "`model` box
...
line
echo "USE AT YOUR OWN RISK! (for details read $CFG_HOME/doc/cfg2html.html)"
echo "Online help: run cfg2html_hpux.sh -h for help or look at the ~/doc directory"
line
...
##  export UNIX95=yes                                       # hint by Gert Leerdam
    ...
    exec_command "cat_and_grep /etc/rc.config.d/* | grep '=[/0-9]'" "Runlevel Settings"
...
#   AddText "Hint: VxFS has unlimited inodes, ninode limit is only valid for HFS file systems!"
    ...
    exec_command "cat_and_grep /etc/fstab" "Mountpoints (fstab)"
...
then # else skip to next paragraph
    ...
    then
        ...
        exec_command "cat_grep /etc/lvmrc" "Auto VG Activation (/etc/lvmrc)"
        # Boot device no longer has to be named vg00  (KL 26.10.11)
        ...
        exec_command "cat_and_grep /stand/bootconf" "Boot Device Configuration Table"
...
then # else skip to next paragraph
    ...
    exec_command "cat_and_grep /etc/rc.config.d/netconf" "Netconf Settings"
    ...
    if [ -f /etc/gated.conf ] ; then
        exec_command "cat_grep /etc/gated.conf" "Gate Daemon Settings"
    fi
    ...
    if [ -f /etc/bootptab ] ; then
        exec_command "cat_and_grep /etc/bootptab" "BOOTP Daemon Configuration"
    fi
...
then # else skip to next paragraph
    ...
    [ -r /etc/syslogd.conf ] && exec_command "cat_and_grep /etc/syslogd.conf" "Syslogd Facility Configuration"
...
then # else skip to next paragraph
    ...
    then
        ...
        [ -r /etc/opt/omni/options/global ] && exec_command "cat_and_grep /etc/opt/omni/options/global" "Nonstandard Global Options"
    ...
    [ -r /opt/omni/.omnirc ] && exec_command "cat_and_grep /opt/omni/.omnirc" "Local DataProtector Client Setting"  # rar, 18.11.2003
    
    ob_instanzen () {
        
        # cat /etc/opt/omni/cell/omni_info | grep '_' |awk '{ printf "%-40s \t %s\n", $1, $2 } '
        ##### changed 06032002 by rar, because of new OB 4.1 format ####
